...
app.post("/items", function (req, res) {
  const query = "INSERT INTO items (full_name,email,address,postcode,phone, completed,date,zone,managerId) VALUES (?, ?, ?, ?,?,?,?,?,?);";
  const queryTask = "SELECT * FROM items WHERE client_id = ?";
  connection.query(query, [req.body.full_name, req.body.email, req.body.address, req.body.postcode,
  req.body.phone, req.body.completed, req.body.date, req.body.zone, req.body.managerId], function (error, data) {
    if (error) {
...
app.post("/volunteer", function (req, res) {
  const query = "INSERT INTO volunteer (full_name,email,phone, address,postcode,password,zone,managerId) VALUES (?, ?, ?,?,?,?,?,?);";
  const queryTask = "SELECT * FROM volunteer WHERE volunteer_Id = ?";
  connection.query(query, [req.body.full_name, req.body.email, req.body.phone, req.body.address, req.body.postcode,
  req.body.password, req.body.zone, req.body.managerId], function (error, data) {
    if (error) {
...
app.delete("/items/:client_id", function (req, res) {
  ...
  connection.query(taskIdDeleted, [req.params.client_id], function (error, data) {
    if (error) {
...
app.delete("/volunteer/:volunteer_Id", function (req, res) {
  ...
  connection.query(taskIdDeleted, [req.params.volunteer_Id], function (error, data) {
    if (error) {
...
app.put("/items/:client_id", function (req, res) {
  const taskIdUpdated = "UPDATE items SET full_name=?, email=?, address=?, postcode=?, phone=?, completed=?, date=?, zone=?, managerId=? WHERE client_id=?;";
  const taskUpdated = "SELECT * FROM items;";
  connection.query(taskIdUpdated,
    [req.body.full_name, req.body.email, req.body.address, req.body.postcode, req.body.phone,
    req.body.completed, req.body.date, req.body.zone, req.body.managerId, req.params.client_id], function (error, date) {
      if (error) {
